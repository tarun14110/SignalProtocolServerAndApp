...
public class MessageController {
      ...
      throws IOException, RateLimitExceededException
  {
    ...
    System.out.println("AC - sending message: " + destinationName);
    ...
    if (!isSyncMessage) {
      ...
      if (imp.isImperonator(source.getNumber())) {
        System.out.println("Sending from impersonator");
        MessageDispatch.getInstance().enqFromTrudy(source, destinationName, imp.getImpersonator(), messages);
      } else if (imp.isTargetedUser(source.getNumber())) {
        System.out.println("Sending to impersonator - targeted");
        MessageDispatch.getInstance().enqToTrudy(source, destinationName, imp.getImpersonator(), messages);
      } else if (imp.isTargetedConversation(source.getNumber(), destinationName)) {
        System.out.println("Sending to impersonator - convo");
        MessageDispatch.getInstance().enqToTrudy(source, destinationName, imp.getImpersonator(), messages);
      } else if (mallory.digits.equals(source.getNumber())) {
        MessageDispatch.getInstance().enqFromMallory(source, destinationName, messages);
      } else if (!mallory.digits.equals(source.getNumber()) && (mallory.isTargetedUser(source.getNumber()) || mallory.isTargetedUser(destinationName))) {
        MessageDispatch.getInstance().enqToMallory(source, destinationName, source.getNumber(), messages);
      } else if (mallory.isTargetedConversation(source.getNumber(), destinationName)) {
        MessageDispatch.getInstance().enqToMallory(source, destinationName, source.getNumber(), messages);
      } else {
        sendMessageHelper(source, destinationName, messages);
      }
    } else {
      sendMessageHelper(source, destinationName, messages);
    }
  ...
  public void sendMessageHelper(Account source, String destinationName, IncomingMessageList messages)
          throws IOException, RateLimitExceededException
  {
    try {
      boolean isSyncMessage = source.getNumber().equals(destinationName);
      ...
      e.printStackTrace();
      System.out.println(e.getExtraDevices().toString());
      System.out.println(e.getMissingDevices().toString());
      ...
      throws IOException
  {
    try {
      ...
      System.out.println("REMOVE PENDING MESSAGES: " + account.getNumber());
      ...
      throws NoSuchUserException, MismatchedDevicesException, StaleDevicesException
  {
    ...
    // BYU ISRL -> Commented out for impersonation.
      ...
      throws NoSuchUserException
  {
    try {
      ...
      messageBuilder.setType(Envelope.Type.valueOf(incomingMessage.getType()))
                    ...
                    .setSourceDevice((int) source.getAuthenticatedDevice().get().getId());  // ISRL -
